;
/* module-key = 'confluence.web.resources:setup-db', location = '/setup/includes/js/setupdb.js' */
...
function(
    ...
    function _getTestConnectionURL() {
        if ($('form[name=setupdbtype]').length > 0) {
            return AJS.contextPath() + '/setup/setupstandarddb-testconnection.action';
        } else {
            return AJS.contextPath() + '/setup/setupdatasourcedb-testconnection.action';
        }
    }
    ...
    function testConnection(event) {
        ...
        $.getJSON(_getTestConnectionURL(), $('form').serialize())
            .done(function(data) {
                if (data.status) {
                    _showSuccessMessage();
                } else {
    ...
    function _showCreatingMessage() {
        setup.showSpinner("We\u0027re just setting up your database. This can take a little while.");
    }
